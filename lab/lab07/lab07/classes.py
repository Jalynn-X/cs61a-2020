...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        # 看到有一种解法是直接用切片再加上新抽出的卡迭代旧的hand
        opponent.hand = opponent.hand[3:] + [opponent.draw() for _ in range(3)] 
...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        attack_to_defense = other_card.defense
        defense_to_attack = other_card.attack
        other_card.attack = attack_to_defense
        other_card.defense = defense_to_attack
        # 网上看到一种解法是直接同时迭代，我以为这样也会有先后顺序然后导致其中一个无法迭代，所以没有这么写
