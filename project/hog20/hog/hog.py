...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        ...
        while n <= max(player_score, opponent_score):
...
def pig_pass(player_score, opponent_score):
    ...
    if player_score >= opponent_score:
        ...
        dif = abs(player_score - opponent_score)
        if dif < 3:
            return True
        return False
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def gain(score0, score1):
        ...
        score_gain = score - last_score
        if score_gain > 0 and score_gain > running_high:
            print(score_gain, "point(s)! The most yet for Player", who)
        high = score_gain if score_gain > running_high else running_high
...
def make_averaged(original_function, trials_count=1000):
    ...
    def cal_avg(*args):
        dice_sum, t = 0, 1
        while t <= trials_count:
            dice_sum = dice_sum + original_function(*args)
            t = t + 1
        average_sum = dice_sum / trials_count
        return average_sum
    return cal_avg
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    dice_num, max_avg = 1, 0
    while dice_num <= 10:
        roll_avg = make_averaged(roll_dice, trials_count)
        cur_avg = roll_avg(dice_num, dice)
        if cur_avg > max_avg:
            max_avg = cur_avg
            max_num = dice_num
        dice_num = dice_num + 1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    #return 6  # Replace this statement
    num_test = 0
    score_test1 = free_bacon(opponent_score)
    score_test2 = score + score_test1
    if extra_turn(score_test2, opponent_score) is True:
        num_rolls = num_test
...
def final_strategy(score, opponent_score):
    ...
    test the win rate of final strategy against the strategy, always_roll(4)
    if the final strategy has a higher win rate, then return True
